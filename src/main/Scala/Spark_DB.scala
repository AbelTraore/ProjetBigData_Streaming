...
object Spark_DB {
  ...
  def main(args : Array[String]): Unit = {
    ...
    val def_mysql = ss.read.jdbc("jdbc:mysql://127.0.0.1:3306/jea_db?zeroDateTimeBehavior=CONVERT_TO_NULL&serverTimezone=UTC","jea_db.orders", props_mysql)
    ...
    val df_mysql2 = ss.read
      ...
      .option("dbtable", "(select state, city, sum(round(numunits * totalprice)) as commandes_totales from orders group by state, city) table_summary")
      .load()
    ...
    //val df_sqlserver = ss.read.jdbc("jdbc:sqlserver://LAPTOP-RMCMAI28\\SPARKSQLSERVER:1433;databaseName=jea_db;","orders",props_SQLServer)

    val df_sqlserver = ss.read
      .format("jdbc")
      .option("driver","com.microsoft.sqlserver.jdbc.SQLServerDriver")
      .option("url", "jdbc:sqlserver://LAPTOP-RMCMAI28\\SPARKSQLSERVER:1433;databaseName=jea_db;integratedSecurity=true")
